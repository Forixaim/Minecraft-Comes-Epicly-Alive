...
This guide is for developers interested in contributing to Minecraft Comes Epicly Alive.
...
- Fork the [GitHub repo](https://github.com/Forixaim/Minecraft-Comes-Epicly-Alive/) to your account. If you already have a fork,
  make sure it's up to date.

* Clone your fork:

    ```bash
    git clone git@github.com:YOUR_GITHUB_USERNAME_HERE/Minecraft-Comes-Epicly-Alive.git
    cd epicfight
    ```
...
* Add the upstream repo:
    ...
    ```bash
    git remote add upstream git@github.com:Epic-Fight/Minecraft-Comes-Epicly-Alive.git
    ```
...
* GitHub will prompt you to open a GitHub pull request, if not, you can
  open [this link](http://github.com/Forixaim/Minecraft-Comes-Epicly-Alive/pull/new).
...
There's no Documentation yet but highly recommend that you document any changes in the codebase.
...
Tight Coupling would lead to a hard dependency which is not good for a mod that only requires just Epic Fight.
...
Mixins are fine but keep them loosely coupled from the main source code. Remember, Mixins are only treated as a LAST RESORT.
...
> However, it's unrelated to MCEA.